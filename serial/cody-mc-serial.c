...
//  ptime(start,"after initpos");

  // Now doing each MC step
  // Equilibration:
  for (int i=0;i<prodstep;i++) {
    // Choosing random atom to move
    int randn = gsl_rng_uniform(r)*natoms;
    ...
    nudge(natompos,natoms,randn,dist,r);
    ...
    if (gsl_rng_uniform(r) < fmin(exp(-(aen-ben)/(kb*t)),1.)) {
      memcpy(atompos,natompos,sizeof(atompos));
      naccept += 1;
    }
  }
  printf("naccept after equilibration: %i, ratio: %f\n",naccept,(float)naccept/(float)prodstep);
  ptime(start,"after eq");

  // Production (with printing):
  for (int i=prodstep;i<nsteps;i++) {
    // Sampling RDF
    if (i%wsteps == 0) {
